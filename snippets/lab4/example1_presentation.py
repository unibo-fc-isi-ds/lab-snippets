...
class Serializer:
    ...
    def _timedelta_to_ast(self, td: timedelta):
        return {"timedelta": self._to_ast(td.total_seconds())}
    ...
    def _response_to_ast(self, response: Response):
        return {
            "result": (
                self._to_ast(response.result) if response.result is not None else None
            ),
...
class Deserializer:
    ...
    def _ast_to_timedelta(self, data):
        return timedelta(seconds=self._ast_to_obj(data["timedelta"]))
    ...
    def _ast_to_response(self, data):
        return Response(
            result=(
                self._ast_to_obj(data["result"]) if data["result"] is not None else None
            ),
...
if __name__ == "__main__":
    from snippets.lab4.example0_users import gc_credentials_wrong, gc_user
