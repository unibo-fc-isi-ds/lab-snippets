...
class ServerStub(Server):
    ...
    def __handle_request(self, request):
        try:
            method = None
            if hasattr(self.__user_db, request.name) and callable(getattr(self.__user_db,request.name)):
                method = getattr(self.__user_db, request.name)
            elif hasattr(self.__auth_service, request.name) and callable(getattr(self.__auth_service, request.name)):
