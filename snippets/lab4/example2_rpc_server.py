...
class ServerStub(Server):
    ...
    def __on_message_event(self, event, payload, connection, error):
        match event:
            case "message":
                ...
                print(
                    "[%s:%d] Unmarshall request:" % connection.remote_address, request
                )
                response = self.__handle_request(request)
                connection.send(serialize(response))
                print(
                    "[%s:%d] Marshall response:" % connection.remote_address, response
                )
                connection.close()
            case "error":
                traceback.print_exception(error)
    ...
    def __handle_request(self, request):
        result = None
        error = f"None of the services can handle the {request.name} request"
        services = [self.__user_db, self.__auth_service]
        try:
