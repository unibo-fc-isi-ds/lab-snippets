...
if __name__ == '__main__':
    ...
    # Creating a novel user with less power (USER) 
    jfletcher = User(
    username='jfletcher',
    emails={'jess.fletcher@unibo.it', 'jessica.fletcher4@gmail.com'},
    full_name='Jessica Fletcher',
    role=Role.USER,
    ...
    # Adding a novel user should work
    try:
        user_db.add_user(jfletcher)
    except RuntimeError as e:
            assert str(e).startswith('User with ID')
            assert str(e).endswith('already exists')

    # Trying to get a user with a non valid token
    try:
        user_db.get_user('gciatto', None)
    except RuntimeError as e:
        #assert 'User with ID gciatto not found' in str(e)
        assert 'Method \'get_user\' need to have a token as an argument.' in str(e)
    ...
    # Authenticating with correct credentials jfletcher user should work
    gc_token_jess = auth_service.authenticate(Credentials(list(jfletcher.ids)[0], jfletcher.password))

    # Getting a normal user (jfletcher) with an admin user (gciatto) by providing a correct token should work
    assert user_db.get_user('jfletcher', gc_token) == jfletcher.copy(password=None)

    # Getting a user with a normal user, by providing a valid token, should work if the users are the same
    assert user_db.get_user('jfletcher', gc_token_jess) == jfletcher.copy(password=None)

    # Getting a different user with a normal user should not work
    try:
        user_db.get_user('gciatto', gc_token_jess) == jfletcher.copy(password=None)
    except RuntimeError as e:
        assert 'A common USER can not get information of a different user.' in str(e)
