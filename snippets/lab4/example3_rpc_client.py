...
class RemoteAuthenticathion(RemoteUserDatabase, AuthenticationService):
    def __init__(self, server_address):
        super().__init__(server_address)
        self.__token=None
    ...
    def check_user_privileges(self, userDb: UserDatabase, credentials: Credentials, token: Token) -> bool:
        if not userDb.check_password(credentials):
        ...
        if token.user.role != Role.ADMIN:
            raise ValueError("You must be Admin to do this operation")
...
if __name__ == '__main__':
    ...
    gc_token = user_authentication.authenticate(gc_credentials_ok[0])
    ...
    # And should be valid
    assert user_authentication.validate_token(gc_token) == True

    # Getting a user that exists while authenticated should work
    assert user_authentication.get_user('gciatto') == gc_user.copy(password=None)
    ...
    for gc_cred in gc_credentials_ok:
        assert user_authentication.check_password(gc_cred) == True

    # Checking credentials should fail if the password is wrong
    assert user_authentication.check_password(gc_credentials_wrong) == False

    # Authenticating with wrong credentials should raise an error
    try:
        user_authentication.authenticate(gc_credentials_wrong)
    except RuntimeError as e:
        assert 'Invalid credentials' in str(e)

    # A token with wrong signature should be invalid
    gc_token_wrong_signature = gc_token.copy(signature='non valid signature')
    assert user_authentication.validate_token(gc_token_wrong_signature) == False

    # A token with expiration in the past should be invalid
    gc_token_expired = user_authentication.authenticate(gc_credentials_ok[0], timedelta(milliseconds=10))
    time.sleep(0.1)
    assert user_authentication.validate_token(gc_token_expired) == False