...
class RemoteAuthenticationService(ClientStub, AuthenticationService):
    def __init__(self, serivce_address):
        super().__init__(server_address=serivce_address)
...
if __name__ == "__main__":
    ...
    from snippets.lab4.example0_users import (
        gc_credentials_ok,
    ...
    auth_service = RemoteAuthenticationService(addr)
    ...
    expiring_token = auth_service.authenticate(
        gc_credentials_ok[0], timedelta(milliseconds=50)
    )
    time.sleep(0.1)
    assert auth_service.validate_token(expiring_token) == False

    for credentials in gc_credentials_ok:
        token = auth_service.authenticate(credentials, timedelta(minutes=15))
