...
if __name__ == '__main__':
    ...
    parser.add_argument('--tokensignature', '-tsign', help='Token signature')
    parser.add_argument('--tokenexpiration', '-texp', help='Token expiration')
    ...
    try :
        ...
        match args.command:
            ...
            case 'authenticate':
                ...
                print(f"expiration: {token.expiration.isoformat()}, signature: {token.signature}")
            case 'validate':
                if not args.tokenexpiration:
                    raise ValueError("Token expiration is required")
                if not args.tokensignature:
                    raise ValueError("Token signature is required")
                token = Token(user_db.get_user(ids[0]), datetime.fromisoformat(args.tokenexpiration), args.tokensignature)
